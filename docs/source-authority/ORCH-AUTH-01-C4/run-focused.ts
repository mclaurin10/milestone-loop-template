...
try {
  ...
  const artifacts = [
    {
      ...
      kind: "source-epoch-focused-vitest-report",
    },
    { path: "test-run-summary.json", kind: "test-run-summary" },
    { path: "execution.json", kind: "source-epoch-focused-execution" },
  ];
  for (const name of ["stdout", "stderr"] as const) {
    ...
    if (bytes.length)
      artifacts.push({
        path: `${name}.log`,
        kind: `source-epoch-focused-${name}`,
      });
  }
  assertCommandPassed(result, "Source epoch snapshot regressions");
  ...
  await writeFile(
    ...
    JSON.stringify(
      {
        schemaVersion: "source-epoch-focused-execution.v1",
