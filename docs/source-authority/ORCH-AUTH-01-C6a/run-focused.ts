...
try {
  ...
  const artifacts = [
    {
      ...
      kind: "source-authority-focused-vitest-report",
    },
    { path: "test-run-summary.json", kind: "test-run-summary" },
    { path: "execution.json", kind: "source-authority-focused-execution" },
    { path: "progress.jsonl", kind: "source-authority-focused-progress" },
    { path: "child-execution.json", kind: "source-authority-child-execution" },
  ];
  for (const name of ["stdout", "stderr"] as const) {
    ...
    if (bytes.length)
      artifacts.push({
        path: `${name}.log`,
        kind: `source-authority-focused-${name}`,
      });
  }
  assertCommandPassed(result, "Source authority regressions");
  ...
  await writeFile(
    ...
    JSON.stringify(
      {
        schemaVersion: "source-authority-focused-execution.v1",
