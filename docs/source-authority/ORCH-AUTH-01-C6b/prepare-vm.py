"""Reuse C3's exact finite host policy for the real full unit check of source readers.
...
assert receiver.name == "vm-3" and os.getuid() == 1000
prior = Path("/home/duncan/oc3-b13lp2vx")
source = repo / "artifacts/wp6e-source-scopes-20260906/vm-3/input"
...
assert metadata["sourceBase"] == "c708945e4ba8e0b75f09bc9e564b4afc5cb47a0a"
assert metadata["projectedTree"] == "12b2e205abab2d728b4c749da8926754acda9686"
root = Path(tempfile.mkdtemp(prefix="oc3-c6b-", dir="/home/duncan"))
payload = root / "input"
payload.mkdir()
...
addition = '''    projection=json.loads(Path('/opt/input/projection.json').read_bytes())
    ...
    run('source-scope-apply-projection',['/usr/bin/git','apply','--index','/opt/input/source.patch'],'/work/source',True)
    projected=run('source-scope-projected-tree',['/usr/bin/git','write-tree'],'/work/source',True).strip()
    ...
    (raw/'source-scope-pnpm-config.yaml').write_bytes(config.read_bytes())
    (raw/'source-scope-pnpm-config.json').write_text(json.dumps({'path':str(config),'sha256':digest(config),'scope':'disposable-guest-account-only','sourceFilesChanged':False})+'\\n')
    environment['LOOP_VERIFY_COMMAND_ARTIFACT_DIR']='/work/source/artifacts/source-scope-unit'
    clockStop=threading.Event()
    def observeClock():
        with (raw/'source-scope-clock-samples.jsonl').open('x') as output:
            while True:
    ...
    clockThread.start()
    try:
        run('source-scope-unit',['/opt/bin/pnpm','test:unit'],'/work/source',True,timeout=1500)
    finally:
        clockStop.set();clockThread.join(timeout=5)
        assert not clockThread.is_alive()
    assert run('source-scope-private-refs-after',['/usr/bin/git','for-each-ref','--format=%(refname)','refs/milestone-loop/'],'/work/source',True).strip()==''
...
(receiver / "guest-program.diff").write_text("".join(difflib.unified_diff(original.splitlines(True), guest.splitlines(True), fromfile="C3/guest-job.py", tofile="source-scopes/guest-job.py")))
...
manifest = {"schemaVersion": "orch-c3-input.v1", "source": {"commit": metadata["sourceBase"], "tree": "aff732d45db883212ceedce09b36fba311265464"}, "files": files}
