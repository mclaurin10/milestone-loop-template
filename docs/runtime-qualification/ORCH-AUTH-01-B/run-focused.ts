...
const files = process.argv
  .slice(2)
  .map((name) => `tools/milestone-orchestrator/src/${name}.test.ts`);
assert(files.length > 0);
const context = await evidenceContext("orch-auth-01-b", "planning-focused");
try {
  ...
  const artifacts = [
    { path: "vitest-report.json", kind: "planning-focused-vitest-report" },
    { path: "test-run-summary.json", kind: "test-run-summary" },
    { path: "execution.json", kind: "planning-focused-execution" },
  ];
  for (const name of ["stdout", "stderr"] as const) {
    const bytes = result[name] ?? "";
    await writeFile(resolve(context.artifactDirectory, `${name}.log`), bytes);
    if (bytes.length)
      artifacts.push({ path: `${name}.log`, kind: `planning-focused-${name}` });
  }
  assertCommandPassed(result, "Planning input and executor regressions");
  ...
  await writeFile(
    ...
    JSON.stringify(
      {
        schemaVersion: "planning-focused-execution.v1",
