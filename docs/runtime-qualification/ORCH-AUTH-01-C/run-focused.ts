...
try {
  ...
  const artifacts = [
    {
      ...
      kind: "host-discovery-focused-vitest-report",
    },
    { path: "test-run-summary.json", kind: "test-run-summary" },
    { path: "execution.json", kind: "host-discovery-focused-execution" },
  ];
  for (const name of ["stdout", "stderr"] as const) {
    ...
    if (bytes.length)
      artifacts.push({
        path: `${name}.log`,
        kind: `host-discovery-focused-${name}`,
      });
  }
  assertCommandPassed(result, "Host discovery regressions");
  ...
  await writeFile(
    ...
    JSON.stringify(
      {
        schemaVersion: "host-discovery-focused-execution.v1",
