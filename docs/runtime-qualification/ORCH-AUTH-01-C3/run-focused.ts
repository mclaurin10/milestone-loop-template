...
const context = await evidenceContext("orch-auth-01-c3", "docker-host-focused");
try {
  ...
  const artifacts = [
    {
      ...
      kind: "docker-host-focused-vitest-report",
    },
    { path: "test-run-summary.json", kind: "test-run-summary" },
    { path: "execution.json", kind: "docker-host-focused-execution" },
  ];
  for (const name of ["stdout", "stderr"] as const) {
    ...
    if (bytes.length)
      artifacts.push({
        path: `${name}.log`,
        kind: `docker-host-focused-${name}`,
      });
  }
  assertCommandPassed(result, "Docker host admission regressions");
  ...
  await writeFile(
    ...
    JSON.stringify(
      {
        schemaVersion: "docker-host-focused-execution.v1",
