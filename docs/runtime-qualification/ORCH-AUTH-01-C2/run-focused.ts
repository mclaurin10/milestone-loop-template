...
try {
  ...
  const artifacts = [
    {
      ...
      kind: "vm-lifecycle-focused-vitest-report",
    },
    { path: "test-run-summary.json", kind: "test-run-summary" },
    { path: "execution.json", kind: "vm-lifecycle-focused-execution" },
  ];
  for (const name of ["stdout", "stderr"] as const) {
    ...
    if (bytes.length)
      artifacts.push({
        path: `${name}.log`,
        kind: `vm-lifecycle-focused-${name}`,
      });
  }
  assertCommandPassed(result, "VM lifecycle regressions");
  ...
  await writeFile(
    ...
    JSON.stringify(
      {
        schemaVersion: "vm-lifecycle-focused-execution.v1",
