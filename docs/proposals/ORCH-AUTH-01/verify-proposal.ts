...
assert(args.length === 0 || (args.length === 1 && args[0] === '--seal'));
assert.equal(process.version, 'v24.18.0');
process.env.LOOP_VERIFY_COMMAND_ARTIFACT_DIR = resolve(dir, 'audit');
const context = await evidenceContext('authority-proposal-integrity', 'ORCH-AUTH-01-document-audit');
...
try {
  ...
  for (const line of status.split(/\r?\n/u).filter(Boolean)) {
    assert(line === '?? "Implementation-ready improvement plan 8-5-26.txt"' || line.startsWith('?? docs/proposals/ORCH-AUTH-01/'), 'Unexpected working-tree change: ' + line);
  }
  ...
  const stages = manifest.plannedCommandSurface.sourceStages;
  assert.equal(stages.length, 11);
  assert.equal(new Set(stages.map((row: any) => row.id)).size, 11);
  assert.deepEqual(stages, commands.sourceStages);
  ...
  const evidenceFiles = (path: string) => path.startsWith('audit/') || ['review-manifest.json', 'proposal-audit.json', 'EXECUTION-PLAN.md'].includes(path);
  ...
  for (const path of reviewFiles) {
    ...
    entries.push({ path, bytes: bytes.length, sha256: sha(bytes), role: path.startsWith('baseline/') || path === 'baseline-index.json' ? 'historical-reference' : /\.(?:ts|mjs)$/u.test(path) ? 'reproduction-tool' : 'normative-proposal' });
  }
  const payload = { schemaVersion: 'authority-proposal-review.v1', revisionId: 'ORCH-AUTH-01', status: 'PROPOSED_NOT_APPROVED', baselineCommit: baseline.commit, baselineTree: baseline.tree, digestAlgorithm: 'sha256(JSON.stringify(payload), UTF-8)', files: entries };
  const contentDigest = sha(JSON.stringify(payload));
  const reviewManifest = { payload, contentDigest };
  if (args[0] === '--seal') await emit('review-manifest.json', reviewManifest);
  else assert.deepEqual(await json('review-manifest.json'), reviewManifest, 'Review bytes changed: do not silently replace an existing approval target');
  ...
  checked('CONTENT-SEAL', 'All proposed JSON parses, review links resolve, and the sorted review inventory binds each normative/historical/reproduction file with its actual size and SHA-256; audit/status outputs are excluded to avoid self-reference.');

  const report = {
    schemaVersion: 'authority-proposal-audit.v1', revisionId: 'ORCH-AUTH-01', status: 'PASS', disposition: 'proposal_consistent', observedAt: new Date().toISOString(),
    baselineCommit: baseline.commit, baselineTree: baseline.tree, workingTreeDirty: status.length > 0,
    proposalContentDigest: contentDigest, approvalReceived: false, activeAuthorityModified: false,
  ...
  await emit('audit/proposal-audit.json', report);
  await emit('audit/review-manifest.json', reviewManifest);
  ...
  assert.deepEqual(await read('audit/proposal-audit.json'), await read('proposal-audit.json'));
  git('diff', '--exit-code');
  git('diff', '--cached', '--exit-code');
  console.log(JSON.stringify({ status: 'PASS', claim: 'proposal document integrity only', contentDigest, ...report.counts, receipt: 'docs/proposals/ORCH-AUTH-01/audit/result.json', completionEligible: false }));
} catch (error) {
