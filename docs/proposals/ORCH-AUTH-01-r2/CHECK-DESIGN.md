...
This is advisory implementation design, subject to the normative source-only contract and APPROVAL-SCOPE.md. Commands below are implementation targets, not commands claimed to exist or pass today. Outcomes and public/trust/cadence contracts are frozen in the proposed normative files. Module paths, proposed new script wiring, internal registry organization and sequencing here may change through normal conformance review.
...
A trusted qualification coordinator freezes the source commit and an invocation nonce, dispatches fresh native Windows/Linux jobs, binds every child artifact to that same qualification identity and exact release digest, and independently checks run/job metadata and archive/receipt hashes. One fresh literal no-argument source `pnpm verify` invocation owns that complete full-qualification matrix. Candidate support declares only the focused cases it needs, is completion-ineligible, and does not trigger the full matrix. Exact closure required by integration/reconciliation remains mandatory. Native jobs run the required stage/workflow commands without recursively running source readiness. A local source-stage PASS is an observation; the source readiness claim stays ineligible until all required platform and provider records are validated. Previous same-commit runs cannot be recycled as fresh exact completion. A control-plane host record and each inner candidate's OCI record are distinct; raw host execution is never mislabeled as `trusted-container`. The advisory [qualification protocol](QUALIFICATION-PROTOCOL.md) describes the proposed dispatch and read-only evidence handoff for both purposes. Normative acceptance owns the provenance, containment, freshness and independent completeness requirements.
...
- The complete release workflow matrix adds build, Git/process/container and native-platform cost. VERIFICATION-CADENCE.md separates this explicit full gate from routine candidate support and distinguishes ordinary single builds from full repeated-build/consumer qualification. It may take longer than current unit-heavy verification. Missing new stages remain non-passing; test caching, broad parallelism, scope suppression and benchmark threshold changes are not included in this approval.
