...
try {
  ...
  const artifacts = [
    {
      ...
      kind: "source-release-focused-vitest-report",
    },
    { path: "test-run-summary.json", kind: "test-run-summary" },
    { path: "execution.json", kind: "source-release-focused-execution" },
  ];
  for (const name of ["stdout", "stderr"] as const) {
    ...
    if (bytes.length)
      artifacts.push({
        path: `${name}.log`,
        kind: `source-release-focused-${name}`,
      });
  }
  assertCommandPassed(result, "Source release regressions");
  ...
  await writeFile(
    ...
    JSON.stringify(
      {
        schemaVersion: "source-release-focused-execution.v1",
