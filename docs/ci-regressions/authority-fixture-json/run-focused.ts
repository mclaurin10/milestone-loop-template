...
try {
  ...
  const artifacts = [
    {
      ...
      kind: "authority-fixture-focused-vitest-report",
    },
    { path: "test-run-summary.json", kind: "test-run-summary" },
    { path: "execution.json", kind: "authority-fixture-focused-execution" },
    { path: "progress.jsonl", kind: "authority-fixture-focused-progress" },
    { path: "child-execution.json", kind: "authority-fixture-child-execution" },
  ];
  for (const name of ["stdout", "stderr"] as const) {
    ...
    if (bytes.length)
      artifacts.push({
        path: `${name}.log`,
        kind: `authority-fixture-focused-${name}`,
      });
  }
  assertCommandPassed(result, "Legacy authority fixture regressions");
  ...
  await writeFile(
    ...
    JSON.stringify(
      {
        schemaVersion: "authority-fixture-focused-execution.v1",
