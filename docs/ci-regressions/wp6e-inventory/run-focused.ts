...
try {
  ...
  const artifacts = [
    {
      ...
      kind: "catalogue-regression-vitest-report",
    },
    { path: "test-run-summary.json", kind: "test-run-summary" },
    { path: "execution.json", kind: "catalogue-regression-execution" },
  ];
  for (const name of ["stdout", "stderr"] as const) {
    ...
    if (bytes.length)
      artifacts.push({
        path: `${name}.log`,
        kind: `catalogue-regression-${name}`,
      });
  }
  assertCommandPassed(result, "Current ownership catalogue regression");
  ...
  await writeFile(
    ...
    JSON.stringify(
      {
        schemaVersion: "catalogue-regression-execution.v1",
