...
The normative approval inventory fixes product outcomes, public contracts, cadence and safety/claim/transition boundaries. Conforming advisory implementation changes receive normal code/evidence review and a refreshed package integrity inventory without renewed approval of unchanged authority. Advisory text cannot override normative meaning. Candidate/iteration evidence never promotes itself to source readiness or substitutes for an exact closure required by integration; routine candidate checks do not automatically dispatch the full native-platform release matrix.

Completion requires the exact clean committed candidate, default profile, matching authority/claim scope, fresh full verification, independently validated artifacts and required platform/provider evidence. Focused, dirty, unavailable, skipped, stale, diagnostic, cross-scope or completion-ineligible results cannot support it. Every successful child verification command writes its owned receipt; exit zero or a self-asserted PASS is insufficient. Missing stages stay non-passing until genuinely implemented. Do not substitute no-op scripts or unit results for public workflow evidence.
