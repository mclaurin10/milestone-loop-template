...
const contractStage = Object.freeze({
  ...
  description: 'Verifies immutable contract hashes, complete requirement sets, and fail-closed gate semantics.',
...
const bootstrapStages = Object.freeze([
  {
    ...
    description: 'Validates required runtimes, exact bootstrap pins, and frozen workspace inputs.',
  ...
  },
  {
    ...
    description: 'Runs formatting, source lint, and dependency-boundary enforcement.',
    acceptanceIds: ['AUTO-01'],
    scripts: ['format:check', 'lint', 'lint:architecture'],
    requiredArtifactKinds: ['format-report', 'lint-report', 'architecture-report'],
    timeoutMs: 300_000,
  },
  {
    id: 'typecheck',
    name: 'Strict type checking',
    description: 'Checks every workspace project under the frozen strict TypeScript policy.',
    acceptanceIds: ['AUTO-01'],
    scripts: ['typecheck'],
  ...
  },
  {
    ...
    description: 'Runs the technical-scaffold Vitest suite through public package boundaries.',
    acceptanceIds: ['AUTO-01'],
    scripts: ['test:unit'],
  ...
  },
  {
    ...
    description: 'Proves a versioned save envelope round trip and deterministic continuation for the smoke state.',
...
const readinessStages = Object.freeze([
  ...
  bootstrapStages[3],
  {
    ...
    description: 'Runs focused package tests and cross-package production-boundary integration tests.',
    acceptanceIds: ['AUTO-01'],
  ...
  },
  {
    ...
    description: 'Compares canonical checkpoints across fresh Node, replay, and Chromium Worker paths.',
    acceptanceIds: ['REPLAY-01'],
    scripts: ['verify:determinism', 'verify:parity', 'verify:replay'],
    requiredArtifactKinds: ['determinism-report', 'parity-report', 'replay-report'],
    timeoutMs: 3_600_000,
  },
  {
    id: 'save-load',
    name: 'Save/load round trips and corruption rejection',
    description: 'Checks continuation parity, required checkpoints, validation, and atomic rejection.',
    acceptanceIds: ['SAVE-01'],
    scripts: ['verify:save'],
  ...
  },
  {
    ...
    description: 'Runs accelerated seasons and required operational/emergency scenarios through production rules.',
  ...
  },
  {
    ...
    description: 'Captures every required visual state at the configured reference viewport.',
    acceptanceIds: ['VIS-01'],
  ...
  },
  {
    ...
    description: 'Collects and evaluates console, page, Worker, request, and renderer diagnostics.',
  ...
  },
  {
    ...
    description: 'Evaluates the fixed reference workloads and records raw timing and memory evidence.',
  ...
  },
  {
    ...
    description: 'Evaluates every original metric, chain, seed, evidence, and readiness gate.',
    acceptanceIds: [
      'AUTO-01',
      'PLAY-01',
      'VIS-01',
...
function parseArguments(argv) {
  ...
  if (options.profileId !== undefined && !Object.hasOwn(PROFILES, options.profileId)) {
    throw new Error(`Unknown verification profile: ${options.profileId}. Expected bootstrap or readiness.`);
  }
...
function printStageList(profileId = undefined) {
  const profiles = profileId === undefined ? Object.values(PROFILES) : [resolveProfile(profileId)];
  for (const profile of profiles) {
    console.log(`${profile.id} - ${profile.name}`);
    for (const stage of profile.stages) {
      const builtIn = stage.kind === 'internal' || stage.kind === 'contract';
      const scriptList = stage.scripts.length > 0 ? stage.scripts.join(', ') : 'none';
...
function resolveProfile(profileId) {
  ...
  if (!profile) {
    throw new Error(`Unknown verification profile: ${profileId}. Expected bootstrap or readiness.`);
  }
  return profile;
}
...
function detectPnpmVersion() {
  const userAgentMatch = process.env.npm_config_user_agent?.match(/(?:^|\s)pnpm\/([^\s]+)/);
...
async function collectCandidateIdentity(packageJson, pnpmVersion) {
  ...
  const gitStatus = commandOutput('git', ['status', '--porcelain=v1', '--untracked-files=all']);
  ...
  return {
    ...
    workspaceManifestSha256: await sha256FileIfPresent(resolve(repositoryRoot, 'pnpm-workspace.yaml')),
    lockfileSha256: await sha256FileIfPresent(resolve(repositoryRoot, 'pnpm-lock.yaml')),
    acceptanceManifestSha256: await sha256FileIfPresent(acceptanceManifestPath),
    immutableContractLockSha256: await sha256FileIfPresent(immutableContractLockPath),
    readinessActivationMarkerSha256: await sha256FileIfPresent(readinessActivationMarkerPath),
...
async function evaluateEnvironment(
  ...
  const { packageJson, error: packageError, exists: packageExists } = packageLoad;
  ...
  const activationCommit = commandOutput(
    'git',
    ['log', '-1', '--format=%H', '--', '.agent/readiness-profile-activated.json'],
  );
  const activationWasCommitted = typeof activationCommit === 'string' && activationCommit.length > 0;
  const lifecycleStatus = profile.id === 'bootstrap'
  ...
  const nodePinIsExact = typeof nodePin === 'string' && /^24\.\d+\.\d+$/.test(nodePin);
  checks.push(
    check(
      'exact-node-pin',
  ...
  checks.push(
    check(
      ...
      packageJson?.scripts?.verify === 'node scripts/verify.mjs' ? STATUS.PASS : STATUS.FAIL,
  ...
  for (const [id, path] of requiredFiles) {
    checks.push(
      check(
        id,
        existsSync(resolve(repositoryRoot, path)) ? STATUS.PASS : STATUS.NOT_READY,
  ...
  for (const [index, scriptName] of stage.scripts.entries()) {
    commands.push(
      await runPackageScript(stage, scriptName, packageJson, artifactRoot, index),
    );
  }
...
async function validateAcceptanceManifest() {
    ...
    try {
      ...
      checks.push(
        check(
          ...
          lockHash === ESTABLISHED_IMMUTABLE_LOCK_SHA256 ? STATUS.PASS : STATUS.FAIL,
      ...
      const lockedPaths = Array.isArray(lock.files) ? lock.files.map((entry) => entry.path) : [];
      ...
      const calibrationStateValid =
        ...
        (
          ...
          (
            ...
            lock.calibrationTransition?.recordPath === 'evals/CALIBRATION_RECORD.md'
          )
        );
      const exactPathSet =
      ...
      if (exactPathSet) {
        ...
        for (const entry of lock.files) {
          ...
          const humanOnlyChanged =
            ...
            entry.activeSha256 !== entry.baselineSha256;
          if (actual !== entry.activeSha256 || openCalibrationChanged || humanOnlyChanged) {
  ...
  try {
    ...
    checks.push(check('manifest-json', STATUS.PASS, 'Acceptance manifest is valid JSON.'));
  } catch (error) {
    return [
      check(
        'manifest-json',
  ...
  const layerIds = new Set(
    Array.isArray(manifest.validationLayers)
      ? manifest.validationLayers.filter((layer) => layer.required).map((layer) => layer.id)
      : [],
  );
  const expectedLayerIds = ['AUTO-01', 'PLAY-01', 'VIS-01', 'PERF-GATE-01', 'REPLAY-01', 'SAVE-01', 'FAULT-01'];
  const exactLayerSet =
    layerIds.size === expectedLayerIds.length && expectedLayerIds.every((id) => layerIds.has(id));
  checks.push(
    check(
  ...
  const botIds = Array.isArray(manifest.botRequirements)
    ...
    : [];
  const exactBotSet =
    botIds.length === expectedBotIds.length && expectedBotIds.every((id) => botIds.includes(id));
  checks.push(
    check(
  ...
  const thresholds = [
    ...completionMetrics.flatMap((metric) => Array.isArray(metric.thresholds) ? metric.thresholds : []),
  ...
  const seedRulesValid =
    ...
    manifest.seedSets?.benchmark?.requireZeroCatastrophicIntegrityFailures === true &&
    manifest.seedSets?.visibleDevelopment?.requireZeroCatastrophicIntegrityFailures === true &&
    manifest.seedSets?.hidden?.requireZeroCatastrophicIntegrityFailures === true;
  checks.push(
    check(
  ...
  checks.push(
    check(
      ...
      plannedVerify?.plannedCommand === 'pnpm verify' ? STATUS.PASS : STATUS.FAIL,
  ...
  try {
    const acceptanceText = await readFile(resolve(repositoryRoot, 'evals', 'ACCEPTANCE.md'), 'utf8');
    checks.push(
      check(
...
async function validateEvidenceReceipt(stage, scriptName, commandArtifactRoot) {
  ...
  if (!existsSync(receiptPath)) {
    return {
      valid: false,
      message: 'Command exited 0 but did not write its required result.json evidence receipt.',
      receipt: null,
      artifacts: [],
    };
  }
  ...
  if (!identityValid || !checksValid) {
    return {
      valid: false,
      message: 'Evidence receipt identity, PASS-only checks, or schema is invalid.',
      receipt,
      artifacts: [],
    };
  }
  ...
  if (artifacts.length === 0) {
    return {
      valid: false,
      message: 'Evidence receipt must declare at least one non-empty command-owned artifact.',
      receipt,
      artifacts: [],
    };
  }
  ...
  for (const artifact of artifacts) {
    ...
    if (!isContainedPath(commandArtifactRoot, artifactPath) || !existsSync(artifactPath)) {
      return {
        valid: false,
    ...
    const artifactHash = artifactStat.isFile() ? await sha256File(artifactPath) : null;
    if (
      !artifactStat.isFile() ||
...
function requiredArtifactChecks(stage, commands) {
  ...
  if (requiredKinds.length === 0 || commands.some((command) => command.status !== STATUS.PASS)) {
    return [];
  }
  const presentKinds = new Set(
    commands.flatMap((command) => command.evidence?.artifacts ?? []).map((artifact) => artifact.kind),
  );
...
async function runPackageScript(stage, scriptName, packageJson, artifactRoot, index) {
  ...
  return new Promise((resolveCommand) => {
    ...
    const capture = (stream, label) => {
      stream.on('data', (chunk) => {
        ...
        process[label === 'stdout' ? 'stdout' : 'stderr'].write(`[${stage.id}] ${text}`);
      });
    };
    ...
    child.on('close', async (exitCode, signal) => {
      ...
      if (!spawnError && !timedOut && exitCode === 0) {
        try {
          evidence = await validateEvidenceReceipt(stage, scriptName, absoluteCommandRoot);
        } catch (error) {
          evidence = {
            valid: false,
...
async function evaluateScriptStage(stage, packageJson, artifactRoot) {
  const started = Date.now();
  const checks = stage.kind === 'contract' ? await validateAcceptanceManifest() : [];
  const commands = [];
  for (const [index, scriptName] of stage.scripts.entries()) {
    commands.push(
      await runPackageScript(stage, scriptName, packageJson, artifactRoot, index),
  ...
  checks.push(...requiredArtifactChecks(stage, commands));
  const statuses = [...checks.map((item) => item.status), ...commands.map((item) => item.status)];
  return {
    id: stage.id,
    name: stage.name,
    description: stage.description,
...
function buildSummary(result) {
  ...
  for (const stage of result.stages) {
    const messages = [
      ...stage.checks.filter((item) => item.status !== STATUS.PASS).map((item) => item.message),
      ...stage.commands.filter((item) => item.status !== STATUS.PASS).map((item) => item.message),
    ];
    lines.push(
...
async function runVerification(options) {
  ...
  if (existsSync(artifactRoot)) {
    throw new Error(`Artifact directory already exists; refusing to overwrite: ${artifactRoot}`);
  }
  ...
  await atomicWriteJson(resolve(artifactRoot, 'run-manifest.json'), runManifest);
  ...
  const stages = [];
  for (const stage of profile.stages.filter((item) => selectedIds.has(item.id))) {
    const result = stage.kind === 'internal'
  ...
  if (status !== STATUS.PASS) completionReasons.push('verification_status_not_pass');
  if (!fullRun) completionReasons.push('focused_stage_selection');
  if (configuredProfileId !== profile.id) completionReasons.push('selected_profile_is_not_package_default');
  if (candidate.workingTreeDirty !== false) completionReasons.push('working_tree_not_proven_clean');
  const completion = {
  ...
  await writeFile(resolve(artifactRoot, 'summary.md'), buildSummary(result), 'utf8');
  ...
  console.log(`[VERIFY] result ${relativeFromRepository(resolve(artifactRoot, 'result.json'))}`);
